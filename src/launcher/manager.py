...
class BaseManager(object):
    ...
    def format_users(self, all_users: list, current_members: list) -> list:
        processed_users = set()
        formatted_users = []
    ...
    def get_project_task_id(self):
        ...
        os.environ[self.cons.launcher_project_id_name] = str(project_id)
        os.environ[self.cons.launcher_task_id_name] = str(task_id)
        return project_id, task_id

    def save_project_task_id(self, project_id, task_id):
        if project_id:
            self.cons.update_config("MainUI", "project_id", str(project_id))
        else:
            self.cons.update_config("MainUI", "project_id", "")
        if task_id:
            self.cons.update_config("MainUI", "task_id", str(task_id))
        else:
    ...
    def get_current_id(self, current_item, config_key):
        ...
        if current_item:
            ...
            if current_id:
                ...
                if config_key == "task_id":
                    if self.view.task_lw.property("pre_task_id"):
...
class ProjectManager(BaseManager):
    def refresh_projects(self):
        pre_task_id = self.cons.configParser.get("MainUI", "task_id", fallback="")
        ...
        def on_success(projects):
            ...
            if projects == self.view.project_lw.property("projects"):
                ...
                return
            self.view.project_lw.clear()
            sorted_projects = sorted(projects, key=lambda x: x["id"])
            ...
            if pre_task_id:
                self.view.task_lw.setProperty("pre_task_id", int(pre_task_id))
                self.cons.switch_task()

        self.run_api_task(
            self.model.get_all_proj,
...
class TaskManager(BaseManager):
    ...
    def add_task(self, title: str, project_id: int = None, parent_id: int = None):
        def on_success(result):
            if result:
    ...
    def delete_task(self, task_name: str, project_id: int, task_id: int):
        def on_success(result):
            if result:
    ...
    def show_add_task_dialog(self):
        ...
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            ...
            if task_name:
                self.add_task(task_name, project_id=project_id)
    ...
    def show_add_subtask_dialog(self):
        ...
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            task_name = dialog.textValue()
            if task_name:
                self.add_task(task_name, project_id=project_id, parent_id=task_id)
    ...
    def show_delete_task_dialog(self):
        ...
        self.delete_task(task_name, project_id=project_id, task_id=task_id)
    ...
    def show_assign_task_dialog(self):
        ...
        def update_member_list(all_users, task_users):
            ...
            dialog.member_list.clear()
            task_member_ids = {member["id"] for member in task_users}
...
class LauncherManager(BaseManager):
    ...
    def refresh_launchers(self, project_id, task_id):
        if not project_id:
            self.view.launcher_lw.clear()
            self.view.launcher_lw.setProperty("launchers", None)
            return
        ...
        def on_success(launchers_data):
            ...
            self.view.launcher_lw.clear()
            sorted_launchers = sorted(launchers_data.items(), key=self.sort_launchers)
            for name, launcher_info in sorted_launchers:
    ...
    def sort_launchers(self, launcher_item):
        _, info = launcher_item
        launcher_id = info.get("id", 0)
        relations = info.get("relations", {})
        tasks = relations.get("tasks", [])
        if not tasks:
            return (0, launcher_id)
        task_id = tasks[0]
        return (1, task_id, launcher_id)
    ...
    def mask_launcher(self, launcher_item):
        ...
        project_id, task_id = self.get_project_task_id()

        current_path = str(project_id)
        if task_id:
            current_task = task_id
            task_path = []
            while current_task:
                task_path.append(str(current_task))
                current_task = self.cons.task_manager.task_parents.get(current_task)
            if task_path:
                current_path = f"{project_id}/{'/'.join(reversed(task_path))}"
        ...
        if not final_status:
            launcher_item.setStyleSheet("QWidget {color: #888888;}")
        else:
    ...
    def add_launcher(self, dialog):
        ...
        def process_uploads():
            if not upload_queue:
                self.run_api_task(
                    self.model.add_launcher,
                    project_id,
                    task_id,
                    software_name,
                    dialog.version_data,
                    success_callback=lambda _: self.refresh_launchers(project_id, task_id),
                    error_callback=lambda e: print(f"Failed to add launcher: {e}"),
                )
                return
    ...
    def delete_launcher(self, launcher_id: int):
        ...
        def on_success(result):
            if result and result.get("success"):
                self.refresh_launchers(project_id, task_id)
    ...
    def show_switch_launcher_dialog(self):
        ...
        project_id, task_id = self.get_project_task_id()
        self.save_project_task_id(project_id, task_id)
        self.refresh_launchers(project_id, task_id)
    ...
    def show_edit_launcher_dialog(self):
        ...
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            self.edit_launcher(launcher_item.launcher_id, dialog)
...
class UserManager(BaseManager):
    ...
    def add_user(self, dialog, parent_dialog):
        ...
        role = self.model.ROLE_MAP.get(dialog.role_combo.currentText().lower(), "member")
    ...
    def edit_user(self, dialog, current_item, parent_dialog):
        ...
        new_role = self.model.ROLE_MAP.get(dialog.role_combo.currentText())
        ...
        self.run_api_task(
            ...
            user_id=user_info["id"],
            username=(new_username if new_username != user_info["username"] else None),
    ...
    def perform_login(self, username, password, dialog=None):
        def on_error(error):
        ...
        def on_success(result):
            if result:
                ...
                self.cons.switch_proj()
                self.cons.switch_mode(int(self.cons.configParser.get("MainUI", "layout", fallback="0")))
            else:
                on_error(result)

        self.run_api_task(
            self.model.auth_user,
            username,
            password,
            success_callback=on_success,
            error_callback=on_error,
...
class IconManager(BaseManager):
    ...
    def _load_resource_icon(self, icon_path: str, icon_label: QtWidgets.QLabel):
        ...
        try:
            ...
            self.run_api_task(
                self.model.auth_service.api_client.get_resource,
                resource_id,
                success_callback=on_success,
    ...
    def upload_icon(self, icon_path: str, on_complete):
        ...
        self.run_api_task(
            self.model.auth_service.api_client.upload_resource,
            icon_path,
            resource_type="image",
            success_callback=on_success,
...
class CleanCachesWorker(QtCore.QRunnable):
    ...
    def run(self):
        try:
            ...
            if not acquire.syncer:
                return
            acquire.syncer.clean_caches(days=7)
        except Exception as e:
